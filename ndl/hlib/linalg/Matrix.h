...
class Matrix {
    public:
       ...
       Vector<Real,N> operator*(Vector<Real,N>& p) const {
      ...
      void transformpoint(Real* p) {
        ...
        void DotProduct(Vector<Real,N> &m,Vector<Real,N> &result){
        ...
        void OuterProduct(Vector<Real,N> a, Vector<Real,N> b){
            Matrix<Real,N> result;
            for (int i=0; i<N; i++){
            for (int j=0; j<N; j++){
                (*this)[i][j] = a[i] * b[j];
        ...
        int EigenDecomposition(Vector<Real,N>& eigenvalues,Matrix<Real,N>& eigenvectors){
        ...
        Matrix<Real,N> &SetScale(Vector<Real,N> &t){
            SetIdentity();
        ...
        Matrix<Real,N> &SetScale(Vector<Real,N-1> &t){
            SetIdentity();
        ...
        //For Homogeneous Coords
        Matrix<Real,N> &SetTranslate(Vector<Real,N-1> &t){
            SetIdentity();
...
class SVD {
  ...
  public:
   ...
   void getSingularValues(Vector<Real,N> &x){ x = s; }
