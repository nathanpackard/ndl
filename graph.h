...
// 	template <class T, size_t N>
// 	using PointCloud = std::vector<Point<T, N>>;
// 
// 	// a Simplex
...
// 		Simplex() { }
// 		double Determinant(){
// 			int n = _min(indices.size(),N+1);
// 			//determinant for area, volume, and hypervolumes
// 			Matrix<double,N+1,N+1> m;
// 			for(int j=0;j<n;j++){
// 			  for(int i=0;i<n;i++){
// 				  if (j==n-1) (m[j])[i]=1;
...
// 		}
// 		double EdgePointDeterminant(std::array<double, N>& p){
...
// 			//3-volume = volume, ...)
// 			int n = _min(indices.size(),N+1);
// 			return Determinant()/(double)_factorial(n);
// 		}
// 
// 		//Simplex Vector Operators
// 		bool operator < (std::array<double, N>& p){ return EdgePointDeterminant(p) < 0; }
// 		bool operator > (std::array<double, N>& p){ return EdgePointDeterminant(p) > 0; }
// 		bool operator <= (std::array<double, N>& p){ return EdgePointDeterminant(p) <= 0; }
// 		bool operator >= (std::array<double, N>& p){ return EdgePointDeterminant(p) >= 0; }
// 		bool operator == (std::array<double, N>& p){ return EdgePointDeterminant(p)==0; }
...
// 		}
// 
// 		Simplex<N>& operator = (Simplex<N>& s){
// 			_points = s._points;
// 		}
// 
// 		//Print out Simplex
