...
void testImageLibraryDimensions()
{
	...
	std::cout << subImage1 << std::endl;
	std::cout << subImage1.CurrentState() << std::endl;
	...
	std::cout << subImage2 << std::endl;
	std::cout << subImage2.CurrentState() << std::endl;
...
void ImageLibrarySpeedTest()
{
...

	{
		...
		{
			for (auto it = roi1.begin(), resultit = result.begin(); it != roi1.end(); ++it, ++resultit)
				*resultit = (it.clampElement(1, 0) + it.clampElement(-1, 0) + it.clampElement(1, 1) + it.clampElement(-1, 1)) * 0.25;
		}, iterations);
...

	{
		...
		{
			for (auto it = roi1.begin(), resultit = result.begin(); it != roi1.end(); ++it, ++resultit)
				*resultit = (it.wrapElement(1, 0) + it.wrapElement(-1, 0) + it.wrapElement(1, 1) + it.wrapElement(-1, 1)) * 0.25;
		}, iterations);
		std::cout << "noAnd is " << (100 * ((basetime / noAnd) - 1)) << "% faster" << std::endl << std::endl;
		std::cout << Image<double, 3>(result, { 0, 0, 0 }, { 4, 4, 1 }, { 1, 1, 1 }, { false, false, false }) << std::endl;
	}
...
void displayBorderTests(Image<unsigned short, 3>& image3D) {
	...
	std::cout << "-2 x clamp: " << image3D.begin().clampElement(-2, 0) << std::endl;
	std::cout << "-2 x wrap: " << image3D.begin().wrapElement(-2, 0) << std::endl;
	std::cout << "-2 x reflect: " << image3D.begin().reflectElement(-2, 0) << std::endl;
	std::cout << "-2 y clamp: " << image3D.begin().clampElement(-2, 1) << std::endl;
	std::cout << "-2 y wrap: " << image3D.begin().wrapElement(-2, 1) << std::endl;
	std::cout << "-2 y reflect: " << image3D.begin().reflectElement(-2, 1) << std::endl;
	std::cout << "-2 z clamp: " << image3D.begin().clampElement(-2, 2) << std::endl;
	std::cout << "-2 z wrap: " << image3D.begin().wrapElement(-2, 2) << std::endl;
	std::cout << "-2 z reflect: " << image3D.begin().reflectElement(-2, 2) << std::endl;
}
